...
describe('<SignUp />', () => {
  it('renders the sign up page with a form submission', async () => {
    ...
    await act(async () => {
      await fireEvent.change(getByPlaceholderText('First Name'), { target: { value: 'Emi' } });
      await fireEvent.change(getByPlaceholderText('Email Address'), { target: { value: 'emicastro@gmail.com' } });
      await fireEvent.change(getByPlaceholderText('Password'), { target: { value: 'password' } });
      fireEvent.click(getByTestId('sign-up'));

      expect(getByPlaceholderText('Email Address').value).toBe('emicastro@gmail.com');
